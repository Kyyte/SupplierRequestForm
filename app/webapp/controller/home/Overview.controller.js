    ...
    return Controller.extend("com.kyyte.procurement.supplierrequestform.home.Overview", {
      ...
      this.getView().setModel(oCheckboxModel, "checkboxModel");
        ...
        oHashChanger.attachEvent("hashChanged", function (oEvent) {
          _this.getView().getModel("undefined").getData().SupplierRequest = {};
          _this.getView().getModel("undefined").refresh(true);
      ...
      _onRouteMatched: function (oEvent) {
        this.getView().getModel("undefined").getData().SupplierRequest = {};
        this.getView().getModel("undefined").refresh(true);
      },
      ...
      onFilterBarSearch: function (oEvent) {
        ...
        aFilters.push(new Filter({
          filters: [
            new Filter({ path: "CategoryID", caseSensitive: false, operator: FilterOperator.Contains, value1: sSearchQuery }),
            new Filter({ path: "CategoryDesc", caseSensitive: false, operator: FilterOperator.Contains, value1: sSearchQuery })
          ],
          and: false
        }));
      ...
      onFilterBarRegionSearch: function (oEvent) {
        var sSearchQuery = this._oBasicSearchField.getValue(),
          aSelectionSet = oEvent.getParameter("selectionSet");
        ...
        aFilters.push(new Filter({
          filters: [
            new Filter({ path: "RegionID", caseSensitive: false, operator: FilterOperator.Contains, value1: sSearchQuery }),
            new Filter({ path: "RegionName", caseSensitive: false, operator: FilterOperator.Contains, value1: sSearchQuery })
          ],
          and: false
        }));
            ...
            var fnSuccess = function (data) {
              ...
              oDataModel.create("/SupplierRequest", oSRRequest, {
                success: (oData) => {
                  ...
                  _this.getView().byId("vcountry").setText("");
      ...
      InitiateDocuSign: function (SRID) {
        ...
        var jdata = {
          SRID: `${SRID}`,
          TemplateID: "f6df1d56-4449-4b5a-9065-028f414dbdce"
        };
      ...
      completedStep1Handler: function (oEvent) {
        ...
        if (Street && SupplierCity && SupplierRegion && PostalCode && Country) {
          ...
          MessageBox.error("Please enter the Address before proceeding to next section");
          this._wizard.setCurrentStep(this.byId("Step1"));
          this._wizard.invalidateStep(this.byId("Step1"));
        }
      ...
      searchAddress: function (oEvent) {

        var inputValue = oEvent.getParameter("newValue");
        var value1 = inputValue.split("%").join("%25");
        var value2 = value1.split("/").join("%2F");
        var value3 = value2.split("?").join("%3F");
        var value4 = value3.split("+").join("%2B");
        var value5 = value4.split("#").join("%23");
        var value6 = value5.split("&").join("%26");
        var value7 = value6.split("'").join("%27%27");

        this._searchField = sap.ui.getCore().byId(oEvent.mParameters.id);
        this.getView().getModel().getData().SupplierRequest.SupplierAddress = inputValue;
        this.getView().getModel().refresh(true);
        var _this = this;

        if (inputValue.length >= 3) {
          var request = new XMLHttpRequest();
          request.open('GET', 'https://zylalabs.com/api/957/address+autocompletion+api/782/autocomplete?text=' + value7 + '&lang=en&countrycodes=us', true);
          request.setRequestHeader("Authorization", "Bearer 4435|OD2B3nkhnxzpKTcyn4nYSqfnP1I5YhjdbyUc1OCQ");
          request.onload = function () {
            ...
            for (var i = 0; i < data.features.length; i++) {
              var array = { "data": data.features[i].properties.formatted };
              values.push(array);
            };
            var oModel = new sap.ui.model.json.JSONModel();
            oModel.setData(values);
            _this._searchField.setModel(oModel, "address");
            _this._searchField.suggest();
          };
          request.send();
        };
      },
      ...
      onAddressSuggest: function (oEvent) {
        ...
        if (sValue) {
          aFilters = [
            new sap.ui.model.Filter([
              new sap.ui.model.Filter("data", function (sDes) {
                //Logic to see for matching words/letters anywhere in string, not just as a continous text
                //return (sDes || "").toUpperCase().indexOf(sValue.toUpperCase()) > -1;
                var searchValue = sValue.toUpperCase().split(" ");
                var searchText = sDes.toUpperCase();
                var found = true;
                if (searchValue) {
                  for (var i = 0; i < searchValue.length; i++) {
                    var text = searchValue[i];
        ...
        if (sap.ui.getCore().byId(oEvent.mParameters.id).getBinding("suggestionItems")) {
          sap.ui.getCore().byId(oEvent.mParameters.id).getBinding("suggestionItems").filter(aFilters);
        };
      },

      onAddressSelected: function (oEvent) {
        var selItempath = oEvent.getParameter("selectedRow").getBindingContextPath().replace("/", "");
        var selData = oEvent.getSource().getModel().getData()[selItempath];
        // uname = selData.Bname;

        // if (uname) {
        //   uname = uname.replace(/#(\S)/g, '$1');
        //   var data = this.getView().getModel("reviewOrder").getData();
        //   //update request for data....
        //   data.RequisitnrId = selData.Bname;
        //   data.RequisitnrName = selData.EmpName;
        //   this.getView().getModel("reviewOrder").refresh(true);
        //   this.updateRequestedForSecChanges();
        // };
      },
