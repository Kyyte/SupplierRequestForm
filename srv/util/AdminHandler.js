...
const { MaterialGroup2Commodity, SupplierOrganization2User } = cds.entities(
  "com.kyyte.procurement.supplierrequestform"
);
...
async function getNextNumber(req) {
  ...
  .from("com.kyyte.procurement.supplierrequestform.NumberRange")
  .where({ Type: SRType });

  const nextNumber = current.CurrentNumber + 1;
  await UPDATE("com.kyyte.procurement.supplierrequestform.NumberRange", {
    Type: SRType
...
async function getSRInfo(req) {
  ...
  let current = await SELECT.from("com.kyyte.procurement.supplierrequestform.SupplierRequest")
  ...
  for (let i =0;i<current.length;i++)
  {
      ...
      let SRStat = await SELECT.one.from("com.kyyte.procurement.supplierrequestform.SRStatus").where({Type:SRType,SuppRequestID:SRNumber});
...
async function getSRStatus(req) {
  ...
  let SRStat = await SELECT.one.from("com.kyyte.procurement.supplierrequestform.SRStatus").where({Type:SRType,SuppRequestID:SRNumberStr});
...
async function doUpdateStatus(req) {
  ...
  await UPSERT.into(
    "com.kyyte.procurement.supplierrequestform.SRStatus"
  ).entries({
    Type: `${SRType}` ,
    SuppRequestID: `${SRID}`,
    Status : `${Status}`
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into(
          "com.kyyte.procurement.supplierrequestform.TailSpendSuppliers"
        ).entries({
          ACMID: `${entity.ACMID}`,
...
async function ACM2ERPMapping(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into(
          "com.kyyte.procurement.supplierrequestform.ACM2ERPMapping"
        ).entries({
          ACMID: `${entity.ACMID}`,
          ERPID: `${entity.ERPID}`,
        });
...
async function Org2UserMapping(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into(
          "com.kyyte.procurement.supplierrequestform.SupplierOrganization2Users"
        ).entries({
    ...
    //  return await srv.create('com.kyyte.procurement.supplierrequestform.SupplierOrganization2Users',entity)
    // }));
...
async function DoLoadCategories(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into(
          "com.kyyte.procurement.supplierrequestform.Categories"
        ).entries({
          CategoryID: `${entity.CategoryID}`,
    ...
    // const results = await Promise.all(entities.map(async (entity) => {
    //  return await srv.create('com.kyyte.procurement.supplierrequestform.Categories',entity)
    // }));
...
// async function DoCreateSupplierfromSR(req) {
//   try {
//     let item = [];
...
//     item.ACMID = req.data.ACMID;
//     item.SupplierName = req.data.SupplierName;
//     item.SupplierStreet = req.data.SupplierStreet;
//     item.SupplierCity = req.data.SupplierCity;
//     item.SupplierRegion = req.data.SupplierRegion;
//     item.SupplierCountry = req.data.SupplierCountry;
//     item.SupplierPostalCode = req.data.SupplierPostalCode;
...
async function DoLoadRegions(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into("com.kyyte.procurement.supplierrequestform.Regions").entries({
          RegionID: `${entity.RegionID}`,
...
async function DoLoadMaterialGroup2Commodity(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        return await UPSERT.into(
          "com.kyyte.procurement.supplierrequestform.MaterialGroup2Commodity"
        ).entries({
...
async function DoLoadCategories2Suppliers(req) {
  try {
    ...
    const results = await Promise.all(
      entities.map(async (entity) => {
        let existingRecord = await SELECT.one
          .from("com.kyyte.procurement.supplierrequestform.Categories2Suppliers")
          .where({
        ...
        if (existingRecord) {
          return await UPSERT.into(
            "com.kyyte.procurement.supplierrequestform.Categories2Suppliers"
          ).entries({
    ...
    // const results = await Promise.all(entities.map(async (entity) => {
    //  return await srv.create('com.kyyte.procurement.supplierrequestform.Categories2Suppliers',entity)
    // }));
...
async function DoPullSuppliersFromAriba(req) {
  ...
  const LastLoadDate = await SELECT.one.from(
    "com.kyyte.procurement.supplierrequestform.SiteControlParams"
  );

  let FromDate;

  console.log(`DoPullSuppliersFromAriba:Inside DoPullSuppliersFromAriba`);
  ...
  console.log(`DoPullSuppliersFromAriba:From Date ${FromDate}`);

  const formData = new FormData();
  formData.append("event", Eventname);
  formData.append("sharedSecret", "OneAribaIntegration1234%");
  formData.append("from", FromDate);
  formData.append("to", CurrentLoadDate);

  console.log(`DoPullSuppliersFromAriba:Current Load  Date ${CurrentLoadDate}`);
  ...
  try {
    ...
    debugger;
    console.log(`DoPullSuppliersFromAriba:Completed axio call`);
    if (response.headers["content-type"] === "application/zip") {
      const zipData = response.data;
      console.log(`DoPullSuppliersFromAriba:start processing Zip`);
      // Process the ZIP file
      const zip = new JSZip();
      const zipContents = await zip.loadAsync(zipData);
      console.log(`DoPullSuppliersFromAriba:completed zip`);
      ...
      if (
        ...
        supplierOrgRows.forEach((row) => {
          ...
          console.log(`DoPullSuppliersFromAriba: inside supplierorgrows ${ACMID}`);
        ...
        //  entities.splice(0, 248);

        // entities.slice(0, 1);

        await doCreateSuppliers(entities);
        ...
        // UPDATE.entity("com.kyyte.procurement.supplierrequestform.SiteControlParams").set({
        ...
        console.error("DoPullSuppliersFromAriba: CSV file not found in the ZIP archive.");
...
async function doCreateSuppliers(entities){
  
  debugger;
  // const entities = req.data.entities;
  const srv = cds.transaction(entities);

  const results = await Promise.all(entities.map(async (entity) => {
   return await srv.create('com.kyyte.procurement.supplierrequestform.Suppliers',entity)
  }));
   
   console.log(`Supplier  ${results.length} entities created successfully.`);
...
async function doGetSupplierDetailDataFromLocalDB(req) {
  try {
    ...
    let PreferredSupplier = await SELECT.one
      .from("com.kyyte.procurement.supplierrequestform.TailSpendSuppliers")
